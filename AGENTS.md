...
- **Prompt structure**: fixed dementia-communication rules first, condition/stage guidance, orientation facts, member list, then caregiver `custom_guidance` (subordinate). Bump `PROMPT_VERSION` when changing wording; it is stored on every session row.
